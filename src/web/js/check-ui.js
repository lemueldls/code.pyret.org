({
  ...
  provides: {},
  nativeRequires: [],
  theModule: function(runtime, _, uri, outputUI, errorUI, option, srcloc, checker, loadLibrary) {

    option = runtime.getField(option, "values")
    srcloc = runtime.getField(srcloc, "values")
    var CH = runtime.getField(checker, "values")

    function isTestResult(value) { return runtime.unwrap(runtime.getField(CH, "TestResult").app(value)) }
    function isTestSuccess(value) { return runtime.unwrap(runtime.getField(CH, "is-success").app(value)) }
    ...
    function drawCheckResults(container, documents, runtime, checkResults, result) {
      ...
      function makeGutterMarker(spanHandle, clickFunction) {
      ...
      var FailingTestSkeleton = function () {
        function FailingTestSkeleton(test, testNumber) {
          ...
          container.append(tombstone)
          var thisTest = this
          var source = get(get(test, "loc"), "source")
          if (documents.has(source)) {
            var document_ = documents.get(source)
            var editor   = document_.getEditor()
            if (editor !== undefined) {
          ...
          if(runtime.hasField(test, "actual-exn")) {
            var stack = get(loadLibrary, "internal")
              .enrichStack(get(test, "actual-exn").val, get(loadLibrary, "internal").getModuleResultRealm(result))
        ...
        FailingTestSkeleton.prototype.refresh = function refresh() {
          ...
          for (const snippet of snippets) {
            window.requestAnimationFrame(
              CodeMirror.prototype.refresh.bind(snippet.CodeMirror))
          }
        }
        ...
        FailingTestSkeleton.prototype.vivify = function vivify(rendering) {
          this.tombstone.append(rendering[0])
      ...
      var PassingTestSkeleton = function () {
        ...
        PassingTestSkeleton.prototype.vivify = function vivify() {
          ...
          this.tombstone.append(snippet.container)
          if (this.block.container.classList.contains("expanded")) {
      ...
      var CheckBlockSkeleton = function () {
        function CheckBlockSkeleton(name, loc, keywordCheck, tests, error) {
          ...
          for (var index_ = 0; index_ < tests.skeletons.length; index_++) {
            var test = tests.skeletons[index_]
            test.block = this
            testFrag.append(test.container)
          }
          ...
          if (error !== undefined) {
            ...
            testList.append(errorTestsSummary)
          } else {
          ...
          header.append(makeNameHandle(name, loc, error !== undefined ? "hsl(0, 100%, 85%)" : (tests.executed == tests.passing ? "hsl(88, 50%, 76%)" : "hsl(45, 100%, 85%)")).anchor)
          ...
          if (error !== undefined) {
            ...
            container.append(tombstone)
            var richStack = get(loadLibrary, "internal")
              .enrichStack(error, get(loadLibrary, "internal").getModuleResultRealm(result)) 
        ...
        CheckBlockSkeleton.prototype.refreshSnippets = function refreshSnippets() {
          for (var index = 0; index < this.needRefreshing.length; index++) {
            this.needRefreshing[index].refresh()
          }
          this.needRefreshing = new Array()
        }
      ...
      for (var index = 0; index < checkBlocks.length; index++) {
        if (get(option, "is-some").app(get(checkBlocks[index], "maybe-err"))) {
          if (get(checkBlocks[index], "keyword-check")) keywordCheck = true
      ...
      try{
        for(var index = checkBlocks.length - 1; index >= 0; index--) {
          var checkBlock = checkBlocks[index]
          ...
          checkResultsContainer.append(skeleton.container)
        }
      ...
      return runtime.safeCall(
        function(){
          return runtime.eachLoop(runtime.makeFunction(function(index) {
            return vivifySkeleton(checkErroredSkeletons[index])
          }), 0, checkErroredSkeletons.length)
        }, function(_) {
          return runtime.safeCall(function() {
            return runtime.eachLoop(runtime.makeFunction(function(index) {
              return vivifySkeleton(testsFailedSkeletons[index])
            }), 0, testsFailedSkeletons.length)
            return runtime.nothing
          }, function(_) {
            for(var index = 0; index < testsPassedSkeletons.length; index++)
              testsPassedSkeletons[index].vivify()
