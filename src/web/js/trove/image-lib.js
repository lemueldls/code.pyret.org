({
  ...
  theModule: function(RUNTIME, NAMESPACE, uri, imageImp, colorLibrary, jsnums, md5) {
    var gf = RUNTIME.getField

    var image = gf(imageImp, "values")
    var imageTypes = gf(imageImp, "types")
    var color = gf(gf(colorLibrary, "values"), "color")
    var annColor = gf(colorLibrary, "types")["Color"] // can't use getField here
    var rawIsColor = gf(gf(colorLibrary, "values"), "is-Color")
    ...
    var makeColor = function(r,g,b,a) {
      if (a === undefined) { a = 1 }
      if ([r,g,b,a].filter(isNumber).length !== 4) {
    ...
    // Color database
    var ColorDatabase = function() {
      this.colors = {}
      this.colorNames = {}
    }

    ColorDatabase.prototype.put = function(name, color) {
      this.colors[name] = color
      var string = // NOTE(ben): Not flooring the numbers here, because they will all be integers anyway
          colorRed(color) + ", " +
          colorGreen(color) + ", " +
          colorBlue(color) + ", " +
          colorAlpha(color)
      if (this.colorNames[string] === undefined) {
        this.colorNames[string] = name
      }
    }

    ColorDatabase.prototype.get = function(name) {
      return this.colors[name.toString().toUpperCase()]
    }

    ColorDatabase.prototype.colorName = function colorName(colorString_) {
      var ans = this.colorNames[colorString_]
    ...
    var colorDatabase = new ColorDatabase()
    var allImageFields = RUNTIME.getFields(gf(colorLibrary, "values"))
    for (var name of allImageFields) {
      var value = gf(gf(colorLibrary, "values"), name)
      name = name.toUpperCase()
      if (isColor(value)) {
        colorDatabase.put(name, value)
        name = name.replace(/-/g, " ")
        colorDatabase.put(name, value)
        name = name.replace(/ /g, "")
        colorDatabase.put(name, value)
    ...
    var isColorOrColorString = function(thing) {
      return (isColor(thing) ||
              ((RUNTIME.isString(thing) &&
                typeof(colorDatabase.get(thing)) != 'undefined')))
    }
    ...
    var colorString = function(aColor, aStyle) {
      var styleAlpha = isNaN(aStyle)? 1 : aStyle,
    ...
    function RGBtoLAB(r, g, b){
      function RGBtoXYZ(r, g, b){
        function process(v){
          v = Number.parseFloat(v/255)
          return (v>0.040_45? Math.pow( (v+0.055)/1.055, 2.4) : v/12.92) * 100
        }
        var variable_R = process(r), variable_G = process(g), variable_B = process(b)
        //Observer. = 2°, Illuminant = D65
        var X = variable_R * 0.4124 + variable_G * 0.3576 + variable_B * 0.1805
        var Y = variable_R * 0.2126 + variable_G * 0.7152 + variable_B * 0.0722
        var Z = variable_R * 0.0193 + variable_G * 0.1192 + variable_B * 0.9505
        return [X, Y, Z]
      }

      function XYZtoLAB(x, y, z){
        var variable_X = x / 95.047           //ref_X =  95.047   Observer= 2°, Illuminant= D65
        var variable_Y = y / 100          //ref_Y = 100.000
        var variable_Z = z / 108.883          //ref_Z = 108.883
        function process(v){ return v>0.008_856? Math.pow(v, 1/3) : (7.787*v) + (16/116) }
        variable_X = process(variable_X); variable_Y = process(variable_Y); variable_Z = process(variable_Z)
        var CIE_L = ( 116 * variable_Y ) - 16
        var CIE_a = 500 * ( variable_X - variable_Y )
        var CIE_b = 200 * ( variable_Y - variable_Z )
        return [CIE_L, CIE_a, CIE_b]
      }
      var xyz = RGBtoXYZ(r,g,b), lab = XYZtoLAB(xyz[0],xyz[1],xyz[2])
      return {l: lab[0], a: lab[1], b:lab[2]}
    }
    var colorLabs = [], colorRgbs = colorDatabase.colors
    ...
    var checkImagePred = function(value) {
      return RUNTIME.isOpaque(value) && isImage(value.val)
    }
    ...
    var verticesEqual = function(v1, v2){
      ...
      var v1_string = v1.map(function(o){return "x:"+o.x+",y:"+o.y}).join(","),
        v2_string = v2.map(function(o){return "x:"+o.x+",y:"+o.y}).join(",")
        // v1 == rot(v2) if append(v1,v1) includes v2
      return (v1_string+","+v1_string).includes(v2_string)
    }
    ...
    BaseImage.prototype.getVertices = function(){
      return this.vertices ? this.vertices : [{x:0 , y: 0},
        {x: this.width, y: 0},
        {x: 0, y: this.height},
    ...
    BaseImage.prototype.render = function(context) {
      if(!this.vertices){
      ...
      if(context.isEqualityTest || isSolid){
      ...
      for (const v of vertices) { context.lineTo( v.x, v.y) }
      context.closePath()

      if (isSolid) {
        context.fillStyle = colorString(this.color, this.style)
        context.fill()
      } else {
        context.strokeStyle = colorString(this.color)
        context.stroke()
      }
      context.restore()
    }
    ...
    BaseImage.prototype.toDomNode = function(parameters) {
      var that = this
    ...
    */
    BaseImage.prototype.difference = function(other) {
      ...
      function rmsDiff(data1,data2){
        var squares = 0
        for(const [index, element] of data1.entries()){
          squares += (element-data2[index])*(element-data2[index])
        }
      ...
      var context1 = c1.getContext('2d'), context2 = c2.getContext('2d')
      this.render(context1)
      other.render(context2)
      try{
        var data1 = context1.getImageData(0, 0, w1, h1),
          data2 = context2.getImageData(0, 0, w2, h2)
        var pixels1 = data1.data,
          pixels2 = data2.data
        return RUNTIME.ffi.makeRight(rmsDiff(pixels1, pixels2))
      } catch(error){
        // if we violate CORS, just bail
        return RUNTIME.ffi.makeLeft("exception: " + String(error))
      }
    }
    ...
    BaseImage.prototype.equals = function(other) {
      ...
      try{
        var context1 = c1.getContext('2d'), context2 = c2.getContext('2d')
        context1.isEqualityTest = true
        context2.isEqualityTest = true
        this.render(context1); other.render(context2)
        // create temporary canvases
        var slice1 = document.createElement('canvas').getContext('2d'),
          slice2 = document.createElement('canvas').getContext('2d')
        var tileW = Math.min(10_000, c1.width) // use only the largest tiles we need for these images
        var tileH = Math.min(10_000, c1.height)
      ...
      // Slow-path can fail with CORS or image-loading problems
        ...
        console.log('Couldn\'t compare images:', error)
        return false
      }
    ...
    var isScene = function(x) {
      return ((x != undefined) && (x != undefined) && (x instanceof SceneImage))
    }
    ...
    var SceneImage = function(width, height, children, withBorder, color) {
      ...
      this.ariaText += children.map(function(c,index){
        return "child "+(index+1)+": "+c[0].ariaText+", positioned at "+c[1]+","+c[2]+" "
      }).join(". ")
    }
    ...
    SceneImage.prototype.render = function(context) {
      var childImage, childX, childY
      // create a clipping region around the boundaries of the Scene
      context.save()
      context.fillStyle = colorString(this.color)
      ...
      for (const child of this.children) { 
        ...
        childY = child[2]
        context.save()
        context.translate(childX, childY)
    ...
    SceneImage.prototype.equals = function(other) {
      return (other instanceof SceneImage     &&
                ...
                this.children.every(function(child1, index) {
                  var child2 = other.children[index]
    ...
    var FileImage = function(source, rawImage) {
      BaseImage.call(this)
      var self = this
      this.src = source
      this.isLoaded = false
      this.ariaText = " image file from "+decodeURIComponent(source).slice(16)
      ...
      if (rawImage && rawImage.complete) {
        ...
        this.img.addEventListener('error', function(e) {
          self.img.addEventListener('error', "")
    ...
    FileImage.installBrokenImage = function(path) {
      imageCache[path] = new TextImage("Unable to load " + path, 10, colorDatabase.get("red"),
        "normal", "Optimer","","",false)
    }

    FileImage.prototype.render = function(context) {
      this.installHackToSupportAnimatedGifs()
      context.drawImage(this.animationHackImg, 0, 0)
    }
    ...
    FileImage.prototype.installHackToSupportAnimatedGifs = function() {
      ...
      document.body.append(this.animationHackImg)
    ...
    var FileVideo = function(source, rawVideo) {
      BaseImage.call(this)
      var self = this
      this.src = source
      this.ariaText = " video file from "+decodeURIComponent(source).slice(16)
      if (rawVideo) {
        this.video			= rawVideo
    ...
    FileVideo.prototype.render = function(context) {
    ...
    ImageDataImage.prototype.render = function(context) {
      ...
      var temporaryCanvas = makeCanvas(this.width, this.height)
      temporaryCanvas.getContext("2d").putImageData(this.imageData, 0, 0)
    ...
    var OverlayImage = function(img1, placeX1, placeY1, offsetX, offsetY, img2, placeX2, placeY2) {
      ...
      var index, v1 = img1.getVertices(), v2 = img2.getVertices(), xs = [], ys = []
    ...
    OverlayImage.prototype.render = function(context) {
      context.save()
    ...
    RotateImage.prototype.render = function(context) {
      context.save()
    ...
    ScaleImage.prototype.render = function(context) {
      context.save()
      context.scale(this.xFactor, this.yFactor)
      if (this.xFactor < 0) {
        context.translate((this.width)/(this.xFactor), 0)
      }
      if (this.yFactor < 0) {
        context.translate(0, (this.height)/(this.yFactor))
      }
      this.img.render(context)
      context.restore()
    }
    ...
    CropImage.prototype.render = function(context) {
      context.save()
      context.beginPath()
      context.rect(0, 0, this.width, this.height)
      context.clip()
    ...
    // scale the context, and pass it to the image's render function
    FrameImage.prototype.render = function(context) {
      context.save()
      this.img.render(context)
      context.beginPath()
      context.strokeStyle = "black"
    ...
    // scale the context, and pass it to the image's render function
    PinholeImage.prototype.render = function(context) {
      context.save()
      context.save()
      this.img.render(context)
      context.restore()
      context.beginPath()
      context.strokeStyle = "black"; context.lineWidth = 1.5
      context.moveTo(this.pinholeX - 5, this.pinholeY)
      context.lineTo(this.pinholeX + 5, this.pinholeY)
      context.moveTo(this.pinholeX, this.pinholeY - 5)
      context.lineTo(this.pinholeX, this.pinholeY + 5)
      context.closePath()
      context.stroke()
      context.beginPath()
      context.strokeStyle = "white"; context.lineWidth = 0.75
      context.moveTo(this.pinholeX - 5, this.pinholeY)
      context.lineTo(this.pinholeX + 5, this.pinholeY)
      context.moveTo(this.pinholeX, this.pinholeY - 5)
      context.lineTo(this.pinholeX, this.pinholeY + 5)
    ...
    FlipImage.prototype.render = function(context) {
    ...
    var RegularPolygonImage = function(length, count, step, style, color, flatBottom) {
      ...
      var numberComponents = jsnums.gcd(count, [step], RUNTIME.NumberErrbacks)
      var pointsPerComponent = count / numberComponents
      var angle = (2*Math.PI/count)
      for (var currentComp = 0; currentComp < numberComponents; currentComp++) {
        radians = currentComp * angle
        for(var index = 0; index < pointsPerComponent; index++) {
      ...
      for (var currentComp = 0; currentComp < numberComponents; currentComp++) {
        ...
        for (var point = 0; point < pointsPerComponent; point++) {
          ...
          component.push(this.vertices[(currentComp * pointsPerComponent) + point])
        }
    ...
    RegularPolygonImage.prototype.render = function(context) {
      var actualVertices = this.vertices
      for (var index = 0; index < this._vertices.length; index++) {
        this.vertices = this._vertices[index]
        BaseImage.prototype.render.call(this, context)
      }
    ...
    var getTextDimensions = function(string, font) {
    ...
    var TextImage = function(string, size, color, face, family, style, weight, underline) {
      BaseImage.call(this)
      this.str        = string
      this.size       = size   // 18
      ...
      var metrics = getTextDimensions(string, this.font)
      ...
      this.ariaText = " the string "+string+", colored "+colorToSpokenString(color,'solid')+" of size "+ size
    }
    TextImage.prototype = heir(BaseImage.prototype)

    TextImage.prototype.render = function(context) {
      context.save()
      context.textAlign   = 'left'
      context.textBaseline= 'alphabetic' // Note: NOT top, so that we can support accented characters
      context.font        = this.font

      // if 'outline' is enabled, use strokeText. Otherwise use fillText
      context.fillStyle = this.outline? 'white' : colorString(this.color)
      context.fillText(this.str, 0, this.alphaBaseline - 1) // handle the baseline offset here
      if(this.outline){
        context.strokeStyle = colorString(this.color)
        context.strokeText(this.str, 0, this.alphaBaseline - 1)
      }
      if(this.underline){
        context.beginPath()
        context.moveTo(0, this.size)
        // we use this.size, as it is more accurate for underlining than this.height
        context.lineTo(this.width, this.size)
        context.closePath()
        context.strokeStyle = colorString(this.color)
        context.stroke()
      }
    ...
    var TriangleImage = function(sideC, angleA, sideB, style, color) {
      ...
      if(thirdY > 0){
        vertices.push({x: offsetX + 0, y: 0}, {x: offsetX + sideC, y: 0}, {x: offsetX + thirdX, y: thirdY})
      } else {
        vertices.push({x: offsetX + 0, y: -thirdY}, {x: offsetX + sideC, y: -thirdY}, {x: offsetX + thirdX, y: 0})
      }
    ...
    EllipseImage.prototype.render = function(context) {
      context.save()
      context.beginPath()
      ...
      var hB = (width  / 2) * 0.552_284_8,
        vB = (height / 2) * 0.552_284_8,
        eX = aX + width,
        eY = aY + height,
        mX = aX + width  / 2,
        mY = aY + height / 2
    ...
    var WedgeImage = function(radius, angle, style, color) {
      ...
      for (var index = 5; index < angle; index += 5) {
        vertices.push({x: radius * Math.cos(index * Math.PI / 180), y: -1 * radius * Math.sin(index * Math.PI / 180)})
      }
    ...
    WedgeImage.prototype.render = function(context) {
      context.save()
      context.beginPath()
      ...
      context.moveTo(aX + this.pinholeX - adjust, aY + this.pinholeY - adjust)
      context.arc(aX + this.pinholeX - adjust, aY + this.pinholeY - adjust, this.radius - 2*adjust, 0, -this.angle, true)
      context.closePath()
      if (this.style.toString().toLowerCase() === "outline") {
        context.strokeStyle = colorString(this.color)
        context.stroke()
    ...
    var LineImage = function(x, y, color) {
      ...
      var vertices
      if (x >= 0) {
        vertices = y >= 0 ? [{x:  0, y:  0}, {x: x, y: y}] : [{x:  0, y: -y}, {x: x, y: 0}]
      } else {
        vertices = y >= 0 ? [{x: -x, y:  0}, {x: 0, y: y}] : [{x: -x, y: -y}, {x: 0, y: 0}]
      }
    ...
    var colorListToImage = function(listOfColors,
      ...
      var canvas = makeCanvas(jsnums.toFixnum(width),
        ...
        context = canvas.getContext("2d"),
        imageData = context.createImageData(jsnums.toFixnum(width),
          jsnums.toFixnum(height)),
        aColor,
        data = imageData.data,
        jsLOC = RUNTIME.ffi.toArray(listOfColors)
      for(var index = 0; index < jsLOC.length * 4; index += 4) {
        aColor = jsLOC[index / 4]
        // NOTE(ben): Flooring colors here to make this a proper RGBA image
        data[index] = Math.floor(colorRed(aColor))
        data[index+1] = Math.floor(colorGreen(aColor))
        data[index+2] = Math.floor(colorBlue(aColor))
        data[index+3] = colorAlpha(aColor) * 255
      }
    ...
    var makeTextImage = function(string, size, color, face, family, style, weight, underline) {
      return new TextImage(string, size, color, face, family, style, weight, underline)
    }
