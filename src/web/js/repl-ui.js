({
  ...
  theModule: function(runtime, _, uri,
    ...
    function closeAnimationIfOpen() {
      for (const animationDiv of animationDivs) {
        animationDiv.empty()
    ...
    function formatCode(container, source) {
      CodeMirror.runMode(source, "pyret", container)
    }
              ...
              left: function(compileResultErrors) {
                ...
                return callingRuntime.safeCall(
                  function() {
                    ...
                    // this application runs in the context of the above `callingRuntime.runThunk`
                    return callingRuntime.eachLoop(runtime.makeFunction(function(index) {
                      // `renderAndDisplayError` must be called in the context of the
                      // pyret stack.
                      return renderAndDisplayError(callingRuntime, errors[index], [], true, result)
                    }), 0, errors.length)
                  }, function (result) {
                    //updateItems(isMain);
                    return result
              ...
              right: function(v) {
                ...
                return callingRuntime.pauseStack(function(restarter) {
                  rr.runThunk(function() {
                    var runResult = rr.getField(loadLibrary, "internal").getModuleResultResult(v)
    ...
    function makeRepl(container, repl, runtime, options) {
      ...
      function outputText(elt) {
        ...
        if (elt.classList.contains('test-results')) {
          ...
          text = ''
          for (var eltI of eltChildren) {
            if (eltI.classList.contains('testing-summary')) {
              var succ = eltI.querySelectorAll('.summary-bits')
              text += succ.length > 0 ? eltI.querySelectorAll('.summary-passed')[0].textContent +
                  ', ' + eltI.querySelectorAll('.summary-failed')[0].textContent + '. ' : eltI.textContent + '. '
              ...
              for (j = 0; j < eltIChildren.length; j++) {
                ...
                if (!eltIJ.classList.contains('check-block-tests')) {
                  text += eltIJ.textContent + '. '
                }
              }
            }
            else {
              text += eltI.textContent + '. '
            }
          }
        } else {
          var ro = elt.querySelectorAll('.replOutput')
          if (ro.length === 0) ro = elt.querySelectorAll('.replTextOutput')
          if (ro.length > 0) text = ro[0].ariaText
          if (!text) text = elt.textContent
        }
        return text
      }
      ...
      runtime.setStdout(function(string) {
        ct_log(string)
        output.append($("<pre>").addClass("replPrint").text(string))
      })
      var currentZIndex = 15_000
          ...
          create: function() {
            ...
            for (const buttonData of buttons) {
              ...
              button.button({icons: {primary: buttonData.icon}, text: false})
                ...
                .css('left', (Number.parseInt(left) + 27) + 'px')
      ...
      runtime.setParam('chart-port', function(arguments_) {
        const animationDiv = $(arguments_.root)
        animationDivs.push(animationDiv)
        output.append(animationDiv)
        ...
        const windowOptions = {
          ...
          height: 'auto',
          beforeClose: () => {
            arguments_.draw(options => $.extend({}, options, {chartArea: null}))
            arguments_.onExit()
            closeTopAnimationIfOpen()
          },
          create: () => {
            ...
            let left = Number.parseInt(titlebar.find("[role='button']:last").css('left'))
            function addButton(icon, function_) {
              left += 27
              const button = $('<button/>')
        ...
        animationDiv
          .dialog($.extend({}, windowOptions, arguments_.windowOptions))
          .dialog('widget')
          .draggable({
            containment: 'none',
            scroll: false,
      ...
      function speakItem(item, isMain) {
        ...
        if (item.erroroutput) {
          ...
          if (isMain) {
            recital += !item.end && !item.start ? ' produced no output.' : ' produced output: '
          } else {
            recital += !item.start ? ' produced no output.' : ' evaluates to '
          }
          var documentOutput = document.querySelector('#output').childNodes
          if (!item.end) {
            if (item.start) {
              recital += outputText(documentOutput[item.start])
            }
          } else {
            //console.log('speakHistory from', item.start, 'to', item.end);
            for (var index = item.start; index < item.end; index++) {
              recital += '. ' + outputText(documentOutput[index])
            }
          }
        }
      ...
      function updateItems(isMain) {
        ...
        var documentOutput = document.querySelector("#output")
        var documentOutputLength = documentOutput.childNodes.length
        var lastOutput = documentOutput.lastElementChild
        ...
        if (lastOutput && lastOutput.classList.contains('compile-error')) {
          ...
          thiscode.start = 0
          thiscode.end = documentOutputLength
      ...
      repl.runtime.setParam("onTrace", function(loc, value, url) {
        if (repl.runtime.getParam("currentMainURL") !== url) { return { "onTrace": "didn't match" } }
        if (repl.runtime.isNothing(value)) { return { "onTrace": "was nothing" } }
        return repl.runtime.pauseStack(function(restarter) {
          repl.runtime.runThunk(function() {
            return repl.runtime.toReprJS(value, repl.runtime.ReprMethods["$cpo"])
          }, function(container) {
       ...
       */
      repl.runtime.setParam("displayRenderer", function(value) {
        return repl.runtime.pauseStack(function(restarter) {
          repl.runtime.runThunk(function() {
            return repl.runtime.toReprJS(value, repl.runtime.ReprMethods["$cpo"])
          }, function(container) {
      ...
      /**
         ...
         @return {!PBase} the value given in
       */
      repl.runtime.setParam("errorDisplayRenderer", function(value) {
        ...
        return repl.runtime.safeCall(function() {
          ...
          */
          ...
          return repl.runtime.safeCall(function() {
            ...
            return repl.runtime.safeCall(function() {
              return repl.runtime.raw_array_map(repl.runtime.makeFunction(function(value) {
                return repl.runtime.toReprJS(value, repl.runtime.ReprMethods["$cpo"])
              }, "spy-to-repr"), vals)
            }, function(rendered) {
              return {
          ...
          function color(index) {
            return outputUI.hueToRGB(palette(index))
          }
          for (const [index, name_] of names.entries()) {
            let row = $("<tr>")
            table.append(row)
            let name = $("<a>").text(name_).addClass("highlight")
            name.attr("title", "Click to scroll source location into view")
            if (locs[index].length === 7) {
              try {
                var pos = outputUI.Position.fromSrcArray(locs[index], CPO.documents, {})
                name.hover((function(pos) {
                  return function() {
                    pos.hint()
                    pos.blink(color(index))
                  }
                })(pos),
                (function(pos) {
                  return function() {
                ...
                name.attr("title", "From " + runtime.getField(runtime.makeSrcloc(locs[index]), "format").app(true))
              }
              // TODO: this is ugly code, copied from output-ui because
              // getting the right srcloc library is hard
              let cmLoc = {
                source: locs[index][0],
                start: {line: locs[index][1] - 1, ch: locs[index][3]},
                end: {line: locs[index][4] - 1, ch: locs[index][6]}
              }
              /*
            ...
            row.append($("<td>").append(rendered[index]))
          }
      ...
      var runMainCode = function(source, uiOptions) {
        if(running) { return }
        history.setToEnd()
        running = true
        output.empty()
        ...
        for (const [name, document] of CPO.documents.entries()) {
        ...
        var replResult = repl.restartInteractions(source, options)
        var startRendering = replResult.then(function(r) {
          maybeShowOutputPending()
          return r
        })
        var doneRendering = startRendering.then(displayResult(output, runtime, repl.runtime, true, updateItems)).fail(function(error) {
      ...
      var runner = function(code) {
        ...
        var doneRendering = startRendering.then(displayResult(output, runtime, repl.runtime, false, updateItems)).fail(function(error) {
          console.error("Error displaying result:", error)
        })
      ...
      CM.on('beforeChange', function(instance, changeObject){textHandlers.autoCorrect(instance, changeObject, CM)})
