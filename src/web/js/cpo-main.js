({
  ...
  theModule: function(runtime, namespace, uri,
    compileLibrary, compileStructs, pyRepl, cpo, replUI, textHandlers,
    parsePyret, runtimeLibrary, loadLibrary, builtinModules, cpoBuiltins,
    ...
    var constructors = gdriveLocators.makeLocatorConstructors(storageAPI, runtime, compileLibrary, compileStructs, parsePyret, builtinModules, cpo)
    ...
    function saveGDriveCachedFile(name, content) {
      var file = storageAPI.then(function(storageAPI) {
        ...
        return existingFile.then(function(f) {
          return f.length > 0 ? f[0] : storageAPI.createFile(name, {
            saveInCache: true,
            fileExtension: ".js",
            mimeType: "text/plain"
          })
      ...
      return runtime.ffi.cases(gmf(compileStructs, "is-Dependency"), "Dependency", dependency,
        {
          ...
          dependency: function(protocol, arguments_) {
            var array = runtime.ffi.toArray(arguments_)
            switch (protocol) {
              case "my-gdrive": {
                return "my-gdrive://" + array[0]
              }
              case "shared-gdrive": {
                return "shared-gdrive://" + array[0] + ":" + array[1]
              }
              case "gdrive-js": {
                return "gdrive-js://" + array[1]
              }
              default: {
                console.error("Unknown import:", dependency)
                return protocol + "://" + array.join(":")
              }
            }
          }
        })
    ...
    function makeFindModule() {
      ...
      function findModule(contextIgnored, dependency) {
        ...
        if(locatorCache.hasOwnProperty(uri)) {
          return gmf(compileLibrary, "located").app(locatorCache[uri], runtime.nothing)
        }
        return runtime.safeCall(function() {
          return runtime.ffi.cases(gmf(compileStructs, "is-Dependency"), "Dependency", dependency,
            {
              ...
              dependency: function(protocol, arguments_) {
                ...
                switch (protocol) {
                  case "my-gdrive": {
                    return constructors.makeMyGDriveLocator(array[0])
                  }
                  case "shared-gdrive": {
                    return constructors.makeSharedGDriveLocator(array[0], array[1])
                  }
                  case "gdrive-js": {
                    return constructors.makeGDriveJSLocator(array[0], array[1])
                  }
                  default: {
          ...
          locatorCache[uri] = l
          return gmf(compileLibrary, "located").app(l, runtime.nothing)
        }, "findModule")
      }
    ...
    var pyRuntime = gf(gf(runtimeLibrary, "internal").brandRuntime, "brand").app(
      runtime.makeObject({
        "runtime": runtime.makeOpaque(runtime)
      }))
    var pyRealm = gf(loadLibrary, "internal").makeRealm(cpoModules.getRealm())


    var builtins = []
    for (const k of Object.keys(runtime.getParam("staticModules"))) {
              ...
              return runtime.safeCall(
                function() {
                  return gf(repl,
                    ...
                    runtime.makeFunction(function() { return string }))
                },
                function(locator) {
    ...
    function withRepl(repl) {
      ...
      window.RUN_CODE = function(source, uiOptions, replOptions) {
        doRunAction(source)
      }
      ...
      */
      function doRunAction(source) {
        ...
        var sheet = document.querySelector("#highlight-styles").sheet
        for(var index=0; index< sheet.cssRules.length; index++) {
          sheet.deleteRule(index)
        }
        switch (currentAction) {
          case "run":
            replWidget.runCode(source, {check: true, cm: editor.cm})
            break
          case "tc-and-run":
            replWidget.runCode(source, {check: true, cm: editor.cm, "type-check": true})
            break
        }
      }
      ...
      */
      ...
      function changeFont(e){
        fontSize = Number.parseInt($('#main').css("font-size"))
      ...
      var currentTheme = document.querySelector("#theme-select").value
      ...
      if (localSettings.getItem('theme') !== null) {
        ...
        localSettings.setItem('theme', currentTheme)
      }
      ...
      editor.cm.on('beforeChange', function(instance, changeObject){textHandlers.autoCorrect(instance, changeObject, editor.cm)})
       ...
       */
      function inferCurrentImg(contents) {
        var query = /img(\d+)\s*=/g
        var maxSoFar = 0
        var res
        ...
        // Picture loaded
          ...
          try {
            photoPrompt().show(function(res) {
              ...
              var origin = "+insertImage" + currentImg
              ...
              var document = cm.getDoc()
              function placeInEditor(string) {
                ...
                document.replaceRange(string, pos, undefined, origin)
                reindent(cursor.line)
              }
              function reindent(line) {
                cm.indentLine(line || document.getCursor().line)
              }
              function emitNewline() {
                var cursor = document.getCursor()
                placeInEditor('\n')
                // FIXME: Dunno why this happens.
                if (cursor.line === document.getCursor().line) {
                  document.setCursor({line: cursor.line + 1, ch: 0})
                }
              }
              function emitLn(s) {
                placeInEditor(s)
              ...
              function onEmptyLine() {
                var cursor = document.getCursor("to")
                var line = document.getLine(cursor.line)
                return (/^\s*$/.test(line))
              }
              ...
              for (const [index, d] of documents.entries()) {
                var pathToImg = '"' + window.APP_BASE_URL + "/shared-image-contents?sharedImageId="
                  + d.id + '"'
                var outstr = asDefs ? ("img" + currentImg + " = ") : ""
                ++currentImg
                outstr += "image-url(" + pathToImg + ")"
                var isLast = (index === (documents.length - 1))
                if (asList) {
                  if (index === 0) {
            ...
            console.error("The show() function failed:", error)
          }
        } else {
