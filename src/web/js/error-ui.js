({
  ...
  provides: {},
  nativeRequires: [ ],
  theModule: function(runtime, _, uri, outputUI, srclocLibrary, errorLibrary, contractsLibrary, checkerLibrary) {
    ...
    var isContractError = function(runtime, v){
      var contracts = runtime.getField(contractsLibrary, "values")
      return runtime.getField(contracts, "is-ContractResult").app(v)
    }

    // NOTE: MUST BE CALLED IN CONTEXT OF PYRET STACK?
    var isRuntimeError  = function(runtime, v){
      var error = runtime.getField(errorLibrary, "values")
    ...
    var isTestResult  = function(runtime, v){
      var error = runtime.getField(checkerLibrary, "values")
    ...
    function applyMethod(runtime, value, name, arguments_) {
      return runtime.
        safeThen(function() {
          return runtime.getField(value, name)
        }, applyMethod).then(function(fun) {
          return fun.app.apply(value, arguments_)
        }).start
    }
