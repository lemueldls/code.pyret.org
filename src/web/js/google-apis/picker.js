  ...
  var showPicker = (function(drive, views, title) {
    ...
    var buildInstance = (function(parentId) {
      ...
      for(const view of views) {
        pickerBuilder.addView(allViews[view])
      }
  ...
  return (function() {
    ...
    return storageAPI.then(function(drive) {
      ...
      .catch((function(error) {
        if (this.raisedError) {
          throw error // <- result from this.onError or this.onInternalError
        } else if (error && error.message) {
          this.onInternalError(error.message)
        } else {
...
FilePicker.prototype.openOn = function(elt, event) {
  elt.addEventListener(event, this.open.bind(this))
}
