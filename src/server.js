...
function start(config, onServerReady) {
  ...
  app.get("/close.html", function (_, res) {
    res.render("close.html");
  });
  app.get("/faq.html", function (_, res) {
    res.render("faq.html");
  });
  app.get("/privacy.html", function (_, res) {
  ...
  app.get("/downloadImg", function (req, response) {
    ...
    var gReq = request(
      { url: googleLink, encoding: "binary" },
      function (error, imgResponse, body) {
        if (error) {
          response.status(400).send({
            type: "image-load-failure",
            error: "Unable to load image " + String(error),
          });
        } else {
          ...
          if (!ct || ct.indexOf("image/") !== 0) {
            response
              .status(400)
              .send({ type: "non-image", error: "Invalid image type " + ct });
            return;
          }
  ...
  app.get(config.google.redirect, function (req, res) {
    auth.serveRedirect(req, function (err, data) {
      if (err) {
        res.send({ type: "auth error", error: err });
      } else {
  ...
  app.get(/\/ide(\/.*)?$/, function (req, res) {
    res.render(path.resolve(__dirname, "web", "ide.html"), {
        ...
        if (err) {
          ...
          drive.permissions.insert(
            ...
            function (err, response) {
              ...
              var sharedProgram = db.createSharedProgram(
                driveFileId,
                u.google_id
              );
  ...
  function getSharedContents(id) {
    ...
    both.then(function (both) {
      ...
      auth.refreshAccess(refreshToken, function (err, newToken) {
        if (err) {
          ret.reject("Could not access shared file.");
          return;
        } else {
          /*
  ...
  app.get("/shared-program-contents", function (req, res) {
    ...
    contents.then(function (response) {
      if (!response.headers["content-type"] === "text/plain") {
        res.status(400);
        res.send(
          "Expected a text file, but got: " + response.headers["content-type"]
        );
        res.end();
      } else {
        response
          .on(
            ...
            (r) =>
              (r.headers[
                "content-disposition"
              ] = `inline; filename="${req.query.sharedProgramId}"`)
          )
          .pipe(res);
      }
    });
        ...
        response
          .on(
            ...
            (r) =>
              (r.headers[
                "content-disposition"
              ] = `inline; filename="${req.query.sharedImageId}"`)
          )
          .pipe(res);
      })
      .fail(function (err) {
        res.status(400);
        res.send(
          "Could not access shared file, or shared file was not an image."
        );
        res.end();
      });
  });
    ...
    both.then(function (both) {
      ...
      auth.refreshAccess(refreshToken, function (err, newToken) {
        if (err) {
          res
            .status(403)
            .send("Couldn't access shared file " + sharedProgramId);
          res.end();
          return;
        } else {
          var drive = getDriveClient(newToken, "v2");
          drive.files.get(
            { fileId: sharedProgramId },
            function (err, response) {
              if (err) {
                res
                  .status(400)
