...
function makeTest(name, imageLibrary, expr, baseurl, badness) {
  return `
...
function makeTests(ARGS) {
  function replaceArguments(strings, ...keys) {
...
describe("Image equality - typed", function () {
  ...
  for (const t of makeTests(TYPED)) {
    var badness = t.length >= 3 ? t[2] : DEFAULT_BADNESS;
    tester.testRunAndAllTestsPass(
      it,
      "image-equality-" + t[0],
...
describe("Image equality - untyped", function () {
  ...
  for (const t of makeTests(UNTYPED)) {
    var badness = t.length >= 3 ? t[2] : DEFAULT_BADNESS;
    tester.testRunAndAllTestsPass(
      it,
      "image-equality-" + t[0],
